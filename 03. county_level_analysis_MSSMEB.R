...
valid_counties <- df_county %>%
  group_by(county) %>%
  filter(sum(!is.na(mean_fuel_price) & !is.na(MSSMEB)) >= 4) %>%
  ungroup()
...
write.xlsx(invalid_counties, "./output/MSSMEB_invalid_counties.xlsx")
...
p_corr_bar <- ggplot(
  ...
  labs(
    ...
    title = "Correlation by county between Petrol-Gasolin Prices and Total MSSMEB",
    x = "County",
...
write.xlsx(invalid_states, "./output/MSSMEB_invalid_states.xlsx")
...
print("Top 2 Counties with highest Fuel/MEB correlation:")
...
p_corr_bar_state <- ggplot(
  ...
  labs(
    ...
    title = "Correlation by state between Petrol-Gasolin Prices and Total MSSMEB",
    x = "State",
    y = "Correlation Coefficient (r)"
  ) +
  theme(
